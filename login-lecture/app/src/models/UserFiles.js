...
const UserStorageFiles = require("./UserStorageFiles");
...
class User {
    ...
    async login() { // async 비동기 함수로 변경
        ...
        const {id, psword} = await UserStorageFiles.getUserInfo(client.id); // body.id == 클라이언트에서 입력한 id 값
        // await 은 Promise를 반환하는 애만 가능하다! 응답이 올 때까지 기다려라!
        // 이 때 이 메소드를 실행하는 시간도 걸리기 때문에 컨트롤러(home.ctrl.js)에도 await을 반드시 걸어줘야 한다.

        if(id) { // id가 UserStorageFiles에 있으면
    ...
    async register() {
        const client = this.body;
        try {
            const responseData = await UserStorageFiles.save(client); // 받은 값 그대로 넘겨주기
            return responseData;
        } catch (err) {
